...
const ROLE_CATEGORIES = [
    { id: 'gtg_staff', label: '👑 Staff Roles (17)', roles: roles_js_1.STAFF_ROLE_NAMES },
...
exports.GtgCommand = {
    ...
    async execute(interaction) {
        ...
        if (!member.permissions.has('ManageRoles')) {
            await interaction.reply({ content: 'You need Manage Roles permission.', flags: discord_js_1.MessageFlags.Ephemeral });
            return;
        }
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setColor(0x00FF00)
            .addFields({ name: '👑 Staff Roles (17)', value: 'Founder, Co-Founder, Lead Dev, Dev, Network Manager, Head Admin, Admin, Sr Mod, Mod, Trial Mod, Head Tester, Sr Tester, Tester, Trial Tester, Support, Builder, Media', inline: false }, { name: '⚔️ Game Mode Tiers (260)', value: '26 modes × 10 tiers each (LT 1 → HT 5)', inline: false }, { name: '🛠️ Utility Roles (4)', value: 'Verified, Member, Muted, Bot', inline: false })
    ...
    async handleButton(interaction) {
        ...
        if (!token) {
            await interaction.reply({ content: 'Bot token not configured.', flags: discord_js_1.MessageFlags.Ephemeral });
            return;
        }
